...
export default function ShortsPage() {
  ...
  // Sort: pinned first, then by displayOrder
  ...
  function togglePin(videoId: string) {
    setShorts((prev) =>
      prev.map((s) =>
        s.videoId === videoId ? { ...s, pinned: !s.pinned } : s
      )
    );
  }

  function toggleHidden(videoId: string) {
    setShorts((prev) =>
      prev.map((s) =>
        s.videoId === videoId ? { ...s, hidden: !s.hidden } : s
      )
    );
  }
  ...
  function moveShort(index: number, direction: "up" | "down") {
    ...
    [newShorts[index], newShorts[targetIndex]] = [
      newShorts[targetIndex],
      newShorts[index],
    ];
  ...
  async function save() {
    ...
    try {
      ...
      const deletePromises = [...existingIds]
        ...
        .map((id) => deleteDoc(doc(db, "shorts", id)));
  ...
  const visibleCount = shorts.filter((s) => !s.hidden).length;
  const pinnedCount = shorts.filter((s) => s.pinned).length;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            Mark videos as shorts and manage display order
          </p>
        </div>
        <button
          onClick={save}
      ...
      {/* Info banner */}
      <div className="rounded-lg border border-saffron/30 bg-saffron/5 px-4 py-3 text-sm text-gray-300">
        <span className="font-semibold text-saffron">Display order:</span>{" "}
        Pinned shorts show first, then by newest date.{" "}
        <span className="text-gray-400">
          {pinnedCount} pinned, {visibleCount} visible, {shorts.length} total
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div>
          <h2 className="mb-3 text-sm font-semibold text-gray-400 uppercase tracking-wider">
            Current Shorts ({shorts.length})
          </h2>
          <div className="space-y-2">
            {shorts.length === 0 ? (
              <div className="rounded-xl border border-border bg-surface p-8 text-center text-gray-500 text-sm">
                No shorts yet. Add videos from the right panel.
              </div>
            ) : (
              sortedShorts.map((s, i) => {
                const realIndex = shorts.findIndex(
                  (x) => x.videoId === s.videoId
                );
                return (
                  <div
                    key={s.videoId}
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-0.5">
                      <button
                        onClick={() => moveShort(realIndex, "up")}
                        disabled={realIndex === 0}
                        className="rounded p-0.5 text-gray-400 hover:text-foreground disabled:opacity-30"
                      >
                      ...
                      <button
                        onClick={() => moveShort(realIndex, "down")}
                        disabled={realIndex === shorts.length - 1}
                        className="rounded p-0.5 text-gray-400 hover:text-foreground disabled:opacity-30"
                      >
                    ...
                    {s.thumbnailUrl ? (
                      ...
                      <div className="flex h-12 w-20 items-center justify-center rounded bg-surface-hover">
                        <IoPlayOutline className="h-5 w-5 text-gray-500" />
                      </div>
                    )}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-1.5 mt-1">
                        {s.pinned && (
                          <span className="rounded bg-saffron/20 px-1.5 py-0.5 text-[10px] font-semibold text-saffron uppercase">
                            Pinned
                          </span>
                        )}
                        {s.hidden && (
                          <span className="rounded bg-red-500/20 px-1.5 py-0.5 text-[10px] font-semibold text-red-400 uppercase">
                            Hidden
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        onClick={() => toggleHidden(s.videoId)}
                        title={s.hidden ? "Show" : "Hide from display"}
                        className={`rounded p-1.5 transition-colors ${
                          s.hidden
